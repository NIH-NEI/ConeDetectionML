...
scanning_resolution = {
    # similar to numpy array shape, first one corresponding to image height, second image width
    'large': [333, 333],
    'small': [128, 128],
}
...
class ao_method():
    ...
    def create_detection_model(self, model_name, model_weight_path):
        ...
        if 'small' in model_name.lower():
            self._scanning_size = scanning_resolution['small']
        else:
            self._scanning_size = scanning_resolution['large']

        str_list = model_name.split('_')
        application = str_list[0]

        self._cone_detection_model = self.create_unet_model(training_size=(training_img_rows, training_img_cols),
                                                            output_class=1)
    ...
    def preprocess_images(self, img):
        ...
        if input_img_size[1] > self._scanning_size[0] and input_img_size[0] > self._scanning_size[1]:
            row_subdivision = input_img_size[1] // self._scanning_size[0]
            col_subdivision = input_img_size[0] // self._scanning_size[1]

            if input_img_size[1] % self._scanning_size[0] != 0:
                row_subdivision = row_subdivision + 1

            if input_img_size[0] % self._scanning_size[1] != 0:
                col_subdivision = col_subdivision + 1;
            ...
            for i in range(row_subdivision):
                if i == row_subdivision - 1 and i * self._scanning_size[0] < input_img_size[1]:
                    row_indices[0] = input_img_size[1] - self._scanning_size[0]
                    row_indices[1] = input_img_size[1]
                else:
                    row_indices[0] = i * self._scanning_size[0]
                    row_indices[1] = (i + 1) * self._scanning_size[0]

                for j in range(col_subdivision):
                    if j == col_subdivision - 1 and j * self._scanning_size[1] < input_img_size[0]:
                        col_indices[0] = input_img_size[0] - self._scanning_size[1]
                        col_indices[1] = input_img_size[0]
                    else:
                        col_indices[0] = j * self._scanning_size[1]
                        col_indices[1] = (j + 1) * self._scanning_size[1]
    ...
    def compute_probablity_map(self, img, normalized_imgs):
        ...
        res_imgs = self._cone_detection_model.predict(normalized_imgs, verbose=1)
        ...
        if res_imgs.shape[0] == 1:
            ...
            for i in range(row_subdivision):
                ...
                for j in range(col_subdivision):
                    ...
                    sub_res_img = resize(sub_res_img, (self._scanning_size[0], self._scanning_size[1]), preserve_range=True)
