...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, parent=None):
        ...
        self.setWindowIcon(qt_icon('ConeDetectionML.png'))
    ...
    def _setup_menu(self):
        self.open_image_act = QtWidgets.QAction('Open...', self, shortcut=QtGui.QKeySequence.Open,
                    icon=qt_icon('open'),
                    statusTip='Open images/annotations',
                    toolTip='Open images/annotations',
    ...
    #     kc = event.key()
    #     mod = event.modifiers()
    #     if not mod & (QtCore.Qt.ControlModifier | mod & QtCore.Qt.AltModifier | mod & QtCore.Qt.MetaModifier):
    #         if kc == QtCore.Qt.Key_Up:
    #             self.previous_image()
    #             return
    #         elif kc == QtCore.Qt.Key_Down:
    #             self.previous_image()
    #             return
    ...
    def _display_help(self):
        ...
        helpFile = os.path.join(HELP_DIR, 'conedetectml.html')
    ...
    def __check_z(self, idx):
        markers = self._input_data['annotations'][idx]
        zmap = defaultdict(int)
        for pt in markers:
            zmap[pt[2]] += 1
        print(self._input_data['image names'][idx], zmap)
    #
    ...
    def _delete_all(self, event):
        id = self._cur_img_id
        if id < 0: return
        if len(self._input_data['annotations'][id]) == 0: return
        ...
        self._file_io.write_points(history_file_name, self._input_data['annotations'][id],
                                   self._input_data['images'][id].GetOrigin(),
                                   self._input_data['images'][id].GetSpacing())
        self._image_view.set_annotations(self._input_data['annotations'][id])
