...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, parent=None):
        ...
        # State dir/file
        self.state_dir = os.path.join(os.path.expanduser('~'), '.ConeDetectionML')
    ...
    def _initialize_input_data(self):
        ...
        self._input_data['annotations'].clear()
    ...
    def _setup_menu(self):
        open_image = QtWidgets.QAction('Open Images...', self, shortcut=QtGui.QKeySequence.Open,
                                      icon=qt_icon('open'),
                                      statusTip='Open images/annotations', triggered=self._open_images)

        open_annotation = QtWidgets.QAction('Open Annotations...', self, shortcut='Ctrl+A',
                                       icon=qt_icon('open_document1'),
                                       statusTip='Open annotations', triggered=self._open_annotations)

        save_data = QtWidgets.QAction('Save Annotations...', self, shortcut=QtGui.QKeySequence.Save,
                                      icon=qt_icon('save'),
                                      statusTip='Save annotations', triggered=self._save_data)
    ...
    def _setup_toolbar(self):
        ...
        save_data_button.setToolTip("Save detection results")
        ...
        detect_button.setToolTip("Detect cones")
        ...
        detect_button.clicked.connect(self._detect_cone_cells)
        ...
        self.annotation_pts_checkbox = QtWidgets.QCheckBox('Annotation visibility')
        self.annotation_pts_checkbox.setChecked(True)
        self.annotation_pts_checkbox.stateChanged.connect(self._set_annotation_points_visibility)
        ...
        detection_setup_layout.addWidget(self.annotation_pts_checkbox, 0, 0, 1, 2)
    ...
    def _open_images(self):
        ...
        file_dialog.setNameFilters(["Images (*.tif)", "Annotations (*.csv)"])
    ...
    def _open_image_list(self, img_filenames):
        ...
        for idx, img_name in enumerate(img_filenames):
            ...
            self._input_data['annotations'].append(annotated_pts)
            self._file_io.write_points(history_file_name, self._input_data['annotations'][idx],
    ...
    def _open_annotations(self):
        ...
        file_dialog.setWindowTitle('Open annotations')
        file_dialog.setFileMode(QtWidgets.QFileDialog.ExistingFiles)
        file_dialog.setWindowFilePath(QtCore.QDir.homePath())
        ...
        for csv_file in csv_filenames:
            ...
            if id != -1:
                try:
                    ...
                    continue
                if self._file_io.is_annotation_spaced(annotated_pts, self._input_data['images'][id]):
                    annotated_pts = self._file_io.scale_annotations(annotated_pts,
                                                                    self._input_data['images'][id])
                self._input_data['annotations'][id] = annotated_pts
    ...
    def _display_image(self, idx):
        ...
        self._image_view.set_annotations(self._input_data['annotations'][idx])

        history_file_name = self.hist.get_history_file(self._input_data['image file paths'][idx])
    ...
    def _detect_cone_cells(self):
        ...
        self._detection_para_dlg.SetImageList(self._input_data['image names'])
        ...
        QtWidgets.QApplication.setOverrideCursor(QtCore.Qt.WaitCursor)
        self._progress_dlg.setWindowTitle('Detecting cones ...')
        self._progress_dlg.show()
        self._progress_dlg.set_progress(0)
        ...
        if not self._cur_img_id in c_rows:
            ...
            self._image_view.set_annotations(self._input_data['annotations'][self._cur_img_id])
